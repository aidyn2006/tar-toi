...
const getPostAuthRedirectPath = () => {
    ...
    try {
        ...
        // Защита от кривых/внешних значений
...
const AuthModal = ({ onClose, defaultMode = 'login' }) => {
    ...
    const validate = () => {
        ...
        if (mode === 'register') {
            ...
            if (!confirmPassword) {
                e.confirmPassword = tr('Құпиясөзді қайталаңыз', 'Повторите пароль');
            } else if (confirmPassword !== password) {
                e.confirmPassword = tr('Құпиясөздер сәйкес келмейді', 'Пароли не совпадают');
            }
        }
    ...
    const handleSubmit = async (ev) => {
        ...
        try {
            ...
            const msg =
                ...
                tr('Қате шықты. Қайталап көріңіз.', 'Произошла ошибка. Попробуйте ещё раз.');
    ...
    return (
        <div
            ...
            onClick={!loading && !status ? onClose : undefined}
            style={{
                position: 'fixed',
                inset: 0,
                zIndex: 1000,
        ...
        >
            <div
                className="home-auth-modal-card"
            ...
            >
                ...
                >
                    ...
                    <p style={{ color: '#64748b', fontSize: '0.9rem', textAlign: 'center' }}>
                        {mode === 'login'
                            ? tr('Аккаунтыңызға кіріңіз', 'Войдите в аккаунт')
                            : tr('Жаңа аккаунт ашыңыз', 'Создайте новый аккаунт')}
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    {mode === 'register' && (
                        <Input
                            ...
                            icon={User}
                            placeholder={tr('Айдын Бекенов', 'Айдын Бекенов')}
                            value={fullName}
                            onChange={(e) => {
                    ...
                    {mode === 'register' && (
                        <Input
                            label={tr('Құпиясөзді растаңыз', 'Повторите пароль')}
                            icon={Lock}
                            type="password"
                            placeholder="••••••••"
